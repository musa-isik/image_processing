...
mpdraw = mp.solutions.draing_utils # çizim için
...
while True:
    ...
    if result.multi_hand_landmarks: # her bir landmarks içerisinde gezilir.
        for handlms in results.multi_hand_landmarks:# handlms ile x y z koordinatları alınır
            mpdraw.draw_landmarks(capture,handlms, mpHand.HAND_CONNECTIONS)
            
            for number,ln in enumerate(handlms.landmark):
                print(number,lm) # number eklem numarası , lm-> x y z koordinatları
                h,w,c = capture.shape
                cx,cy = int(lm.x*w),int(lm.y*h)
                
                #işaret parmağı tespit
